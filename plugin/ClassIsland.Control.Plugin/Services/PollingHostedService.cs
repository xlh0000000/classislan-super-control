    ...
    ILogger<PollingHostedService> logger) : BackgroundService
{
    ...
    private async Task EnrollAsync(CancellationToken cancellationToken)
    {
        ...
        var response = await client.EnrollAsync(new EnrollmentRequest(store.Settings.EnrollmentToken, store.Settings.DeviceName, publicKeyJwk, null, "0.1.2", AppBase.AppVersion, $"{AppBase.Current.OperatingSystem}/{AppBase.Current.Platform}"), cancellationToken);
