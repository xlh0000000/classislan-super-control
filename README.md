...
| 🗓 课表 | 可视化排课与常驻快速选择器；排好的档案存进配置库，由策略引用后生效 |
| 📚 配置库 | 档案 / 组件布局 / 自动化 / 插件设置 四类配置，可视化编辑器，可导入现有 JSON；只维护内容，交付统一走策略 |
| 📣 任务编排 | 显式设备 / 组织子树 / 标签三种目标解析，支持分批灰度、暂停、取消与重试 |
| 🎲 点名 | 管理端维护名单，并可远程决定设备是否开点名窗、抽中是否提醒、抽人停留时长；设置按设备 → 组织 → 全校逐字段继承，随轮询与名单一起增量下发 |
