...
export type SessionUser = { id: string; username: string; displayName: string; role: string; scopeOrgNodeId: string | null };
...
export function getSessionUser(event: Parameters<typeof getCookie>[0]): SessionUser | null {
  const token = getCookie(event, "cic_session");
  if (!token) return null;
  const row = useDatabase().prepare(`SELECT u.id, u.username, u.display_name as displayName, u.role, u.scope_org_node_id as scopeOrgNodeId, s.id as sessionId
    FROM sessions s JOIN users u ON u.id=s.user_id
    WHERE s.token_hash=? AND s.expires_at>? AND u.disabled_at IS NULL`).get(sha256(token), nowIso()) as (SessionUser & { sessionId: string }) | undefined;
  if (!row) return null;
  useDatabase().prepare("UPDATE sessions SET last_seen_at=? WHERE id=?").run(nowIso(), row.sessionId);
  return { id: row.id, username: row.username, displayName: row.displayName, role: row.role, scopeOrgNodeId: row.scopeOrgNodeId ?? null };
}

export function requireUser(event: Parameters<typeof getCookie>[0]) {
