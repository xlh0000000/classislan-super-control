...
describe("点名名单下发", () => {
  it("没有任何名单时设备拿到空名单与修订 0", () => {
    const db = createDb();
    expect(resolveRollCallForDevice(db, DEVICE_1)).toEqual({ revision: 0, names: [] });
    db.close();
  });
  ...
  it("全校名单对所有设备生效", () => {
    ...
    for (const deviceId of [DEVICE_1, DEVICE_2, DEVICE_3])
      expect(resolveRollCallForDevice(db, deviceId)).toEqual({ revision: saved.revision, names: ["张三", "李四"] });
    db.close();
  });
  ...
  it("按 设备 → 最近的组织 → 全校 的优先级解析", () => {
    ...
    save(db, "device", DEVICE_1, ["设备专属"]);
    expect(resolveRollCallForDevice(db, DEVICE_1).names).toEqual(["设备专属"]);
    db.close();
  });
